...
cadis_throughput = [28.82, 11.80, 7.41, 4.25, 2.30]

plt.figure(figsize=(10, 4))

plt.plot(latencies, parallax_throughput, marker='o', label='Parallax', color='skyblue', linestyle='-', markeredgecolor='black')
plt.plot(latencies, cadis_throughput, marker='s', label='CADIS', color='orange', linestyle='-', markeredgecolor='black')

# Add numbers next to each point
for i, (latency, p_val, c_val) in enumerate(zip(latencies, parallax_throughput, cadis_throughput)):
    # Parallax numbers below the point
    plt.text(latency - 20, p_val - 0.3, f'{p_val:.2f}', ha='center', va='top', fontsize=10, color='darkblue')
    # CADIS numbers above the point
